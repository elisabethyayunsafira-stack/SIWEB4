...
export default function LoginPage() {
  ...
  const [id, setId] = useState(""); const [key, setKey] = useState("");
  const [sync, setSync] = useState(false); const [connecting, setConnecting] = useState(false);
  const [status, setStatus] = useState("WAITING FOR INPUT..."); const [statusColor, setStatusColor] = useState("#a855f7");
  const [error, setError] = useState("");

  const handleConnect = () => {
    if (connecting) return;
    if (!id.trim() || !key.trim()) { setError("OPERATOR ID & AUTHORIZATION KEY REQUIRED"); setStatus("INPUT ERROR"); setStatusColor("#f87171"); return; }
    setError(""); setConnecting(true); setStatus("AUTHENTICATING..."); setStatusColor("#f59e0b");
    setTimeout(() => { setStatus("CONNECTION ESTABLISHED — REDIRECTING..."); setStatusColor("#22c55e"); setTimeout(() => router.push("/dashboard"), 800); }, 2500);
  };
          ...
          <div className="modal">
            ...
            <div className="field">
              ...
              <div className="input-wrap">
                ...
                <input type="text" value={id} onChange={e=>{setId(e.target.value);setError("")}} onKeyDown={e=>e.key==="Enter"&&handleConnect()} placeholder="Alpha-9-Delta"/>
              </div>
            </div>
            <div className="field">
              ...
              <div className="input-wrap">
                ...
                <input type="password" value={key} onChange={e=>{setKey(e.target.value);setError("")}} onKeyDown={e=>e.key==="Enter"&&handleConnect()} placeholder="············"/>
              </div>
            </div>
            {error&&<div className="err-box"><span className="err-text">⚠ {error}</span></div>}
            <div className="row-opt">
              ...
              <a href="#" className="key-rec">KEY RECOVERY</a>
            </div>
            ...
            <div className="status-row">
              ...
              <span className="stext" style={{color:statusColor=="#22c55e"?"#22c55e":statusColor=="#f59e0b"?"#f59e0b":statusColor=="#f87171"?"#f87171":"#6b7280"}}>{status}</span>
            </div>
          </div>
        </div>

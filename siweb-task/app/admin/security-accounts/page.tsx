...
export default function SecurityAccountsPage() {
  ...
  const [actionNote, setActionNote] = useState("READY FOR PROTOCOL UPDATE");
  const authLetters = ["S","I","G","N"];
  ...
  const pushLog = (msg: string, color: string) => {
    ...
    setLogEntries((current) => [{ time, msg, color }, ...current].slice(0, 5));
  };

  const handleForceLogout = (name: string) => {
    setSessionList((current) => current.filter((session) => session.name !== name));
    setActionNote(`${name} SESSION TERMINATED`);
    pushLog(`${name} force logout executed`, "#f87171");
  };

  const handleTerminateNonEssential = () => {
    setSessionList((current) => current.filter((session) => session.role === "ROOT"));
    setActionNote("NON-ESSENTIAL SESSIONS TERMINATED");
    pushLog("All non-essential sessions terminated", "#a855f7");
  };

  const handleExecuteOverride = () => {
    if (!adminId.trim() || !newKey.trim() || !confirmKey.trim()) {
      setActionNote("ALL CREDENTIAL FIELDS ARE REQUIRED");
      pushLog("Credential override rejected: missing fields", "#f87171");
      return;
    }

    if (newKey !== confirmKey) {
      setActionNote("PROTOCOL KEY CONFIRMATION MISMATCH");
      pushLog("Credential override rejected: confirmation mismatch", "#f87171");
      return;
    }

    setActionNote("OVERRIDE EXECUTED SUCCESSFULLY");
    pushLog(`Credential override executed by ${adminId}`, "#22c55e");
  };
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .ph-stat-val{font-family:'Orbitron',sans-serif;font-size:12px;font-weight:700}
        ...
        .exe-btn{width:100%;height:48px;background:linear-gradient(90deg,#c9f1ff 0%,#d48cff 100%);border:none;border-radius:4px;color:#2b2230;font-family:'Orbitron',sans-serif;font-size:11px;font-weight:700;letter-spacing:0.22em;cursor:pointer;margin-bottom:14px;position:relative;overflow:hidden}
        ...
        .action-note{margin-top:10px;font-family:'Share Tech Mono',monospace;font-size:8px;letter-spacing:0.14em;color:#22d3ee;text-transform:uppercase}
      ...
      <div className="layout3">
        <div className="lc3">
            ...
            <div className="session-card" key={i}>
              ...
              <div className="session-right">
                ...
                <button
                  type="button"
                  className="force-btn"
                  onClick={() => handleForceLogout(s.name)}
                >
                  FORCE LOGOUT
                </button>
              </div>
          ...
          <button
            ...
            onClick={handleTerminateNonEssential}
          >
            TERMINATE ALL NON-ESSENTIAL SESSIONS
          </button>
          <div className="action-note">{actionNote}</div>
        </div>
        <div className="rc3">
          {/* Credential Control */}
